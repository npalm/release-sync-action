...
export async function run() {
    try {
        ...
        if (sourceRepoParts.length !== 2) {
            throw new Error(`Invalid source repository format, expected owner/repo, got ${sourceRepo}`);
        }
        ...
        if (targetRepoParts.length !== 2) {
            throw new Error(`Invalid target repository format, expected owner/repo, got ${githubRepository}`);
        }
        const target = {
        ...
        if (!sourceExists) {
            throw new Error(`Source repository ${source.owner}/${source.repo} does not exist`);
        }
        const targetExists = await checkRepoExists(target, octokit);
        if (!targetExists) {
            throw new Error(`Target repository ${target.owner}/${target.repo} does not exist`);
        }
        // get releases from source
        const result = await octokit.paginate(octokit.rest.repos.listReleases, {
            ...source
        });
        core.info(`Found ${result.length} releases in ${source.owner}/${source.repo}`);
        ...
        for (const release of releases) {
            if (start || release.tag_name == startFrom) {
                ...
                core.info(`Skipping release ${release.tag_name}, starting from ${startFrom}`);
                continue;
            }
            await checkRateLimit();
            // check release exists in target
            ...
            try {
                ...
                core.info(`Release ${release.tag_name} exists in ${target.owner}/${target.repo}`);
                if (deleteReleases) {
                    core.info(`Deleting release ${release.tag_name} in ${target.owner}/${target.repo}`);
                    ...
                    core.info(`Skipping release ${release.tag_name} in ${target.owner}/${target.repo}`);
                    continue;
                }
            }
            catch (error) {
...
async function checkRateLimit() {
    ...
    core.info(`Rate limit remaining: ${rateLimit.data.rate.remaining}, limit: ${rateLimit.data.rate.limit}`);
    if (rateLimit.data.rate.remaining < rateLimit.data.rate.limit * 0.2) {
        throw new Error(`Rate limit almost exhausted, remaining: ${rateLimit.data.rate.remaining}, limit: ${rateLimit.data.rate.limit}`);
    }
}
async function copyRelease(target, release) {
    core.info(`Copying release ${release.tag_name} to ${target.owner}/${target.repo}`);
...
async function checkRepoExists(repo, octokit) {
    try {
